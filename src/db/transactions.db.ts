...
import { transactions, trades, transfers, addressBalanceCache, AddressBalance } from "./schema.ts";
...
export async function listHistoricAddressBalances(refresh?: boolean): Promise<AddressBalance[]> {
    ...
    if (refresh) {
        ...
        await cacheAddressBalances(res.rows);
...
export async function cacheAddressBalances(addressBalances?: AddressBalance[]): Promise<void> {
    if (!addressBalances)
        addressBalances = await listHistoricAddressBalances(true);

    if (addressBalances.length) {
        await db.delete(addressBalanceCache);

        for (let i = 0; i < addressBalances.length; i += 20) {
            const batch = addressBalances.slice(i, i + 20);
            
            await db.insert(addressBalanceCache).values(batch);
        }
    }
}